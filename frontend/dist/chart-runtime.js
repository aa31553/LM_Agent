var LMFlintBundle = (() => {
  ...
  // frontend/src/chart-runtime.js
  var chart_runtime_exports = {};
  __export(chart_runtime_exports, {
    compile: () => compile,
    render: () => render,
    supportsFlint: () => supportsFlint,
    toFlintInput: () => toFlintInput
  });

  // node_modules/flint-chart/dist/echarts/index.js
  ...
  function convertTemporalData(data, semanticTypes2) {
    if (data.length === 0) return data;
    const keys = Object.keys(data[0]);
    const temporalKeys = keys.filter((k) => {
      const st = toTypeString(semanticTypes2[k]);
    ...
    return values.map((r) => {
      for (const temporalKey of temporalKeys) {
        ...
        const st = toTypeString(semanticTypes2[temporalKey]);
        if (typeof val === "number") {
  ...
  function resolveChannelSemantics(encodings, data, semanticTypes2, convertedData) {
    const result = {};
    const temporalData = convertedData ?? data;
    for (const [channel, encoding2] of Object.entries(encodings)) {
      const fieldName = encoding2.field;
      if (!fieldName && encoding2.aggregate !== "count") continue;
      if (!fieldName && encoding2.aggregate === "count") {
        result[channel] = {
          field: "_count",
      ...
      const rawAnnotation = semanticTypes2[fieldName];
      ...
      if (encoding2.aggregate) {
        if (encoding2.aggregate === "count") {
          cs.field = "_count";
          cs.type = "quantitative";
        } else {
          cs.field = `${fieldName}_${encoding2.aggregate}`;
          cs.type = "quantitative";
        }
      }
      if ((channel === "color" || channel === "group") && fieldName) {
        if (encoding2.scheme && encoding2.scheme !== "default") {
          cs.colorScheme = {
            scheme: encoding2.scheme,
            type: "categorical",
      ...
      if (cs.type === "ordinal" || cs.type === "nominal") {
        if (!encoding2.sortOrder && !encoding2.sortBy) {
  ...
  var ecBumpChartDef = {
    ...
    instantiate: (spec, ctx) => {
      const { channelSemantics, table, semanticTypes: semanticTypes2 } = ctx;
      const xCS = channelSemantics.x;
      const yCS = channelSemantics.y;
      const colorField = channelSemantics.color?.field;
      if (!xCS?.field || !yCS?.field) return;
      const xField = xCS.field;
      const yField = yCS.field;
      const ySemType = toTypeString(semanticTypes2?.[yField]);
      const xSemType = toTypeString(semanticTypes2?.[xField]);
  ...
  function makeOutlierSeries(name, data, laneIndex, laneCount, horizontal) {
    return {
      ...
      encode: { tooltip: [0, 1] },
      z: 3,
      renderItem: (_params, api2) => {
        const category = Number(api2.value(0));
        const value = Number(api2.value(1));
        const point = horizontal ? api2.coord([value, category]) : api2.coord([category, value]);
        const size = api2.size(horizontal ? [0, 1] : [1, 0]);
  ...
  function makeGroupSeparatorSeries(categoryCount, horizontal) {
    return {
      ...
      tooltip: { show: false },
      z: 0,
      renderItem: (params, api2) => {
        const index = Number(api2.value(0));
        const current = horizontal ? api2.coord([0, index]) : api2.coord([index, 0]);
        const next = horizontal ? api2.coord([0, index + 1]) : api2.coord([index + 1, 0]);
      ...
      const option = {
        ...
        series: [
          ...
          // ECharts splits positive/negative values onto separate stacks).
          {
            ...
            encode: { x: 0, y: [1, 2] },
            renderItem: (params, api2) => {
              const i = api2.value(0);
              const lo = api2.value(1);
              const hi = api2.value(2);
              const pLo = api2.coord([i, lo]);
              const pHi = api2.coord([i, hi]);
              const band = api2.size([1, 0])[0];
              ...
              const pTip = api2.coord([i, tipVals[idx]]);
              const children = [rect];
              children.push({
                type: "text",
                style: {
          ...
          // Connector lines: gap-only horizontal segments between adjacent bars.
          {
            ...
            renderItem: (_params, api2) => {
              const i = api2.value(0);
              const y = api2.value(1);
              const pThis = api2.coord([i, y]);
              const pNext = api2.coord([i + 1, y]);
  ...
  var ecGanttChartDef = {
    ...
    declareLayoutMode: () => ({ axisFlags: { y: { banded: true } } }),
    instantiate: (spec, ctx) => {
      const { channelSemantics, table, chartProperties, semanticTypes: semanticTypes2 } = ctx;
      ...
      const temporal = isGanttTemporal(channelSemantics.x?.type, semanticTypes2[startField]);
  ...
  var defaultOverflowStrategy = (channel, fieldName, uniqueValues, maxToKeep, context) => {
    ...
    const encoding2 = encodings[channel];
    const sortBy = encoding2?.sortBy;
    const sortOrder = encoding2?.sortOrder;
    let sortField;
    let sortFieldType;
  ...
  function decideColorForChannel(channel, ctx) {
    const encoding2 = ctx.encodings[channel];
    const cs = ctx.channelSemantics[channel];
    if (!encoding2 || !cs?.field) return void 0;
  ...
  function normalizeStaticSeries(rawEncodings, data, semanticTypes2) {
    ...
    for (const entry of entries) {
      ...
      if (!explicitType && data.length > 0) {
        const semType = semanticTypes2[field];
  ...
  function assembleECharts(input) {
    ...
    const semanticTypes2 = input.semantic_types ?? {};
    ...
    const prelimConvertedData = convertTemporalData(data, semanticTypes2);
    ...
    const convertedData = convertTemporalData(data, semanticTypes2);
  ...
  function buildECEncodings(encodings, channelSemantics, declaration, data, canvasSize, semanticTypes2, templateMarkType, chartTemplate) {
    const resolved = {};
    const encodingsEntries = Object.entries(encodings);
    for (const [channel, encoding2] of encodingsEntries) {
      const entry = {};
      const fieldName = encoding2.field;
      const cs = channelSemantics[channel];
      if (channel === "radius") {
      ...
      if (fieldName) {
        ...
        if (encoding2.aggregate) {
          if (encoding2.aggregate === "count") {
            ...
            entry.field = `${fieldName}_${encoding2.aggregate}`;
            entry.type = "quantitative";
          }
        }
      ...
      if (encoding2.sortBy || encoding2.sortOrder) {
        entry.sortOrder = encoding2.sortOrder;
        entry.sortBy = encoding2.sortBy;
        if (encoding2.sortBy) {
          if (encoding2.sortBy === "x" || encoding2.sortBy === "y" || encoding2.sortBy === "color") ;
          else {
            try {
              if (fieldName) {
                const fieldSemType = toTypeString(semanticTypes2[fieldName]);
                const fieldVisCat = inferVisCategory(data.map((r) => r[fieldName]));
                let sortedValues = JSON.parse(encoding2.sortBy);
                if (fieldVisCat === "temporal" || fieldSemType === "Year" || fieldSemType === "Decade") {
                  sortedValues = sortedValues.map((v) => String(v));
                }
                entry.sortValues = encoding2.sortOrder === "descending" ? [...sortedValues].reverse() : sortedValues;
              }
            } catch {
            }
          }
  ...
  // frontend/src/lm-chart-to-flint.js
  var CHART_TYPES = Object.freeze({
    bar: "Bar Chart",
  ...
  function toFlintInput(chart, size = {}) {
    ...
    if (chart.data.length > 5e3) throw new RangeError("chart.data exceeds the frontend point limit");
  ...
  var api = Object.freeze({ compile, render, supportsFlint, toFlintInput });
  globalThis.LMFlintCharts = api;
  return __toCommonJS(chart_runtime_exports);
})();
