  ...
  $("#chat").innerHTML = `
    <div class="grid two">
      ...
      <div class="panel">
        ...
        <div id="answerSections" class="answer-sections" hidden></div>
        <div id="usageOutput" class="usage-summary" hidden></div>
      </div>
      <div class="panel full-width">
        <div class="panel-header"><h2>回應</h2></div>
...
function renderChatResponse(payload) {
  ...
  renderAnswerSections(payload?.sections);
  renderUsage(payload?.usage);
}

function renderAnswerSections(sections) {
  const container = $("#answerSections");
  container.replaceChildren();
  const entries = [
    ["重點", sections?.key_points],
    ["來源", sections?.sources],
    ["信心", sections?.confidence ? [sections.confidence] : []],
    ["限制", sections?.limitations],
  ];
  for (const [title, values] of entries) {
    if (!Array.isArray(values) || values.length === 0) continue;
    const block = document.createElement("section");
    const heading = document.createElement("h3");
    heading.textContent = title;
    block.appendChild(heading);
    if (values.length === 1 && title === "信心") {
...
function renderUsage(usage) {
  const container = $("#usageOutput");
  const values = [
    ["輸入", usage?.prompt_tokens],
    ["輸出", usage?.completion_tokens],
    ["總計", usage?.total_tokens],
  ].filter(([, value]) => Number.isInteger(value));
  container.textContent = values.map(([label, value]) => `${label} ${value} tokens`).join(" · ");
  ...
  try {
    ...
    while (true) {
      ...
      for (const part of parts) {
        ...
        if (data.response) {
          renderChatResponse(data.response);
